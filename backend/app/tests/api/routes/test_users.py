...
from app.models.order import Account
...
from app.tests.utils.utils import random_email, random_lower_string, delete_account
...
def test_create_user_new_email(
    ...
    with patch("app.utils.send_email", return_value=None), patch(
        ...
        # Delete data created
        delete_account(db, db.get(Account, user.id))
...
def test_retrieve_users(
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    u1 = crud.user.create_user(session=db, user_create=user_in)

    username2 = random_email()
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    u2 = crud.user.create_user(session=db, user_create=user_in2)
...
def test_create_user_open(client: TestClient, db: Session) -> None:
    with patch("app.core.config.settings.USERS_OPEN_REGISTRATION", True):
        ...
        # Delete data created
        delete_account(db, db.get(Account, created_user["id"]))
